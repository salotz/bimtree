...
#+BEGIN_SRC bash
git clone https://github.com/salotz/bimtree
cd bimtree
#+END_SRC
...
To create an env called ~bimtree.dev~ just run the ~env~ target from
~invoke~:
...
#+begin_src bash
pip install --index-url https://test.pypi.org/simple/ --no-deps bimtree-salotz
#+end_src
